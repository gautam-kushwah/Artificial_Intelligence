...
class AgentFunction {
	...
	int nextmove(WumpusWorld wumpusWorld) {
		List<WumpusCellClass> possibleCells = new ArrayList<WumpusCellClass>();
		List<WumpusCellClass> possibleUnexploredCells = new ArrayList<WumpusCellClass>();
		List<WumpusCellClass> possibleExploredCells = new ArrayList<WumpusCellClass>();
		List<Integer> result = new ArrayList<>();
		int destX = agentXPos;
		int destY = agentYPos;

		if(agentXPos != 0) {
			// if agent is not in the first cell
			// and cell to the left is safe or has no possibility of pit or wumpus
			if (wumpusWorld.cells[agentXPos - 1][agentYPos].safe != -1 && (wumpusWorld.cells[agentXPos - 1][agentYPos].safe == 1 || (wumpusWorld.cells[agentXPos - 1][agentYPos].wumpus == 0 && wumpusWorld.cells[agentXPos - 1][agentYPos].pit == 0) )) {
				
				WumpusCellClass wumpus = new WumpusCellClass();
		...
		if (agentXPos != 3) {
			// if agent is not at the right edge
			// and cell to the right is safe or has no possibility of pit or wumpus
		...
		if(agentYPos != 0) {
//			if agent is not at the bottom edge
			// and cell below is either safe or has no possibility of wumpus and pit
		...
		if (agentYPos != 3) {
			// if agent is not at the top edge
			//and cell above is either safe or has no possibility of wumpus and pit
			if (wumpusWorld.cells[agentXPos][agentYPos + 1].safe != -1
					&& (wumpusWorld.cells[agentXPos][agentYPos + 1].safe == 1
					|| (wumpusWorld.cells[agentXPos][agentYPos + 1].wumpus == 0
					&& wumpusWorld.cells[agentXPos][agentYPos + 1].pit == 0) )) {
		...
		// if possible cells to move are 0, stay where the agent is
		if (possibleCells.size() == 0) {
				destX = agentXPos;
				destY = agentYPos;
		} else if (possibleCells.size() == 1) {
			//if it's just one then move to that cell
			for (WumpusCellClass possiblecell : possibleCells) {
				destX = possiblecell.x;
				destY = possiblecell.y;
			}
		} else {
			//else divide cells into explored and unexplored cells
			for (WumpusCellClass possiblecell : possibleCells) {
	...
	int nextMovement(int destinationX, int destinationY, WumpusWorld wumpusWorld) {
		//if destination cell is on the right
		if(agentXPos < destinationX) {
		...
		} else if(agentXPos > destinationX) { //if destination cell is on the left
			if (direction == 'W') {
				agentXPos = destinationX;
				wumpusWorld.cells[agentXPos][agentYPos].visited = 1;
				wumpusWorld.cells[agentXPos][agentYPos].safe = 1;
		...
		} else { //if destination cell is below
			...
			} else if (agentYPos > destinationY) { //if destination cell is above
				if (direction == 'S') {
					agentYPos = destinationY;
					wumpusWorld.cells[agentXPos][agentYPos].visited = 1;
					wumpusWorld.cells[agentXPos][agentYPos].safe = 1;
